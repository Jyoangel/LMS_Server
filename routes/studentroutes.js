...
router.get('/count', async (req, res) => {
    try {
        ...
        const count = await StudentDetail.countDocuments({ userId });

        res.status(200).json({ message: `The total number of students for user ${userId} is: ${count}`, count });
    } catch (error) {
...
// Route to send messages to all students, teachers, and staff
router.post('/sendAllMessages', async (req, res) => {
    const { subject, message, userId } = req.body; // Include userId in the request body
    ...
    try {
        // Find students, teachers, and staff by userId
        const students = await StudentDetail.find({ userId });
        const teachers = await Teacher.find({ userId });
        const staff = await Staff.find({ userId });

        // Combine the results into a single array
        const allUsers = [...students, ...teachers, ...staff];

        if (allUsers.length === 0) {
            return res.status(400).json({ message: 'No users found for messaging with the provided userId' });
        }

        // Extract emails of all users
        const emails = allUsers.map(user => user.email).filter(email => email); // Ensure we filter out any undefined or null emails

        if (emails.length === 0) {
            return res.status(400).json({ message: 'No valid email addresses found for messaging' });
        }

        // Configure email options
        const mailOptions = {
            from: EMAIL_USER,
            to: emails.join(','), // Join emails into a comma-separated list
            subject: subject,
            text: message
        };
        ...
        transporter.sendMail(mailOptions, async (error, info) => {
            ...
            const sentMessage = new Message({
                subject,
                message,
                recipientCount: emails.length // Optional: Save the number of recipients
            });
